...
describe("world weather renderer state", () => {
    ...
    test("keeps fog clear around the player and fades distant visibility instead of covering the whole map", () => {
        ...
        expect(belowMinimum.hiddenRadius).toBeCloseTo(visualMinimum.hiddenRadius);
        expect(belowMinimum.maxAlpha).toBeCloseTo(visualMinimum.maxAlpha);
        expect(reference.clearRadius).toBeGreaterThanOrEqual(6);
        expect(reference.clearRadius).toBeLessThanOrEqual(7);
        expect(reference.fadeRadius).toBeGreaterThanOrEqual(14);
        expect(reference.fadeRadius).toBeLessThanOrEqual(15);
        expect(reference.hiddenRadius).toBeGreaterThanOrEqual(24);
        expect(reference.hiddenRadius).toBeLessThanOrEqual(25);
        expect(reference.silhouetteAlpha).toBeGreaterThanOrEqual(0.78);
        expect(reference.silhouetteAlpha).toBeLessThan(0.8);
        expect(reference.maxAlpha).toBeGreaterThanOrEqual(0.98);
        expect(reference.maxAlpha).toBeLessThan(1);
        expect(reference.color).toBe(0xb4c8cd);
        expect(dense.fadeRadius).toBeGreaterThan(dense.clearRadius + 7);
        expect(dense.hiddenRadius).toBeGreaterThan(dense.fadeRadius + 8);
        expect(dense.maxAlpha).toBeLessThanOrEqual(0.99);
        ...
        const outlineAlpha = getWorldFogFalloffSampleAlpha(reference.fadeRadius, reference);
        const hiddenAlpha = getWorldFogFalloffSampleAlpha(reference.hiddenRadius + 1, reference);
        ...
        expect(midAlpha).toBeLessThan(reference.silhouetteAlpha);
        expect(outlineAlpha).toBe(reference.silhouetteAlpha);
        expect(hiddenAlpha).toBe(reference.maxAlpha);
        expect(getWorldFogFalloffSampleAlpha(dense.hiddenRadius + 10, dense)).toBe(dense.maxAlpha);
